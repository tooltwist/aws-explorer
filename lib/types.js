...
module.exports.id = function (node) {
  ...
  switch (node.type) {
    ...
    case this.VPC:
      if (!node.data) console.log(`ZUMBO 99 Missing node.data`);
...
function describeVpc (node) {
  ...
  if (!node.data) console.log(`WUMBO 1 - missing node.data. node is`, node);
    
  s += `   Id: ${node.data ? node.data.VpcId : '?'}<br>`
  s += `State: ${node.data ? node.data.State : '?'}<br>`
